...
export const AppLayout = ({ children, headerTitle, headerSubtitle, onNewIssueClick, customHeaderActions }) => {
  ...
  const getNavLinks = () => {
    if (role === 'admin') {
      return [
        ...
        { path: '/admin?tab=map', label: 'Issues Map', icon: MapPin },
        { path: '/admin?tab=analytics', label: 'Analytics', icon: BarChart2 },
        { path: '/admin?tab=departments', label: 'Departments', icon: Building2 },
        { path: '/admin?tab=logs', label: 'System Logs', icon: FileText },
  ...
  const brandName = 'CivicFlow';
  const brandSubtitle = role === 'admin' ? 'Admin Portal' : 'Smart Redressal';
