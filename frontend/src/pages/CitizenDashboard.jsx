...
export const CitizenDashboard = () => {
  ...
  const [allNearbyIssues, setAllNearbyIssues] = useState([]);
  ...
  const fetchCitizenData = async () => {
    ...
    try {
      const [userComplaintsRes, allIssuesRes] = await Promise.all([
        complaintService.getComplaints().catch(() => ({ complaints: [] })),
        complaintService.getComplaints({ limit: 50 }).catch(() => ({ complaints: [] }))
      ]);

      setComplaints(userComplaintsRes.complaints || []);
      setAllNearbyIssues(allIssuesRes.complaints || []);
    } catch (err) {
